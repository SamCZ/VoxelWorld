    ...
    public Chunk getChunk(int x, int z)
    {
        ...
        for (int bx = 0; bx < 16 + 2; bx++)
        {
            for (int bz = 0; bz < 16 + 2; bz++)
            {
                ...
                if (biome > 0.4)
                {
                    height += scaled_octave_noise_2d(6, 1.85f, 0.25f, 0, 10, fx * 0.1f, fz * 0.45f) * 10.0f;
                }

                this.noiceCache[bx, bz] = height;
            }
        }

        int bilinearPassthroughtCount = 2;

        for (int i = 0; i < bilinearPassthroughtCount; i++)
        {
            for (int bx = 1; bx < 16 + 1; bx++)
            {
                for (int bz = 1; bz < 16 + 1; bz++)
                {
        ...
        for (int bx = 0; bx < 16; bx++)
        {
            for (int bz = 0; bz < 16; bz++)
            {
                ...
                int height = Mathf.Clamp(Mathf.FloorToInt(this.noiceCache[bx + 1, bz + 1]), 0, 256);
